...
import { useResendOtp, useVerifyUserOtp } from "../hooks/auth-query";
import { toast } from "sonner";
import { isAxiosError } from "axios";
import { Spinner } from "@/components/atoms/spinner";
import Link from "next/link";

const VerifyUserOtpForm = () => {
    const router = useRouter()
    const verifyUserOtpForm = useForm<VerifyUserOtpSchemaType>({
        defaultValues: {
            otp: "",
        },
    ...
    const { verifyUserOtpMutation } = useVerifyUserOtp()
    ...
    useEffect(() => {
        const savedEndTime = localStorage.getItem('resendOtpEndTime');
        if (savedEndTime) {
    ...
    const onSubmit = async (data: VerifyUserOtpSchemaType) => {
        const payload = {
            email_or_phone: userEmail || '',
            otp: data.otp,
        }
        await verifyUserOtpMutation.mutateAsync(payload, {
            onSuccess: (response) => {
                ...
                <div className="w-full p-8 shadow-lg">
                    <div className="flex flex-col items-center mb-8">
                        <div className="flex items-center gap-2">
                            <ArrowLeft onClick={() => router.push('/auth/sign-up')} className="cursor-pointer" />
                            <h1 className="text-3xl font-bold mb-2">Verify Your OTP</h1>
                        </div>
                        <p>Enter the OTP sent to your email</p>
                    </div>

                    <Form {...verifyUserOtpForm}>
                        <form
                            onSubmit={verifyUserOtpForm.handleSubmit(onSubmit)}
                            className="space-y-4 max-w-fit mx-auto"
                        >
                            <div className="flex flex-col items-end">
                                <FormField
                                    control={verifyUserOtpForm.control}
                                    name="otp"
                                    render={({ field }) => (
                                ...
                                <Button
                                    type="button"
                                    variant={'link'}
                                    className="mt-6 p-0 cursor-pointer"
                            ...
                            <Button
                                type="submit"
                                disabled={verifyUserOtpForm.formState.isSubmitting}
                                className=" h-11 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-md"
                            >
                                {
                                    verifyUserOtpForm.formState.isSubmitting ? <div className="flex">Verifying...... <Spinner /></div> : <div>Verify</div>
                                }
                            </Button>
                        </form>
                    </Form>
...
export default VerifyUserOtpForm;
