...
import { useCreateGroupPostStore, GroupPostType } from "../stores/create-group-post-store";
...
import { GroupTextPostForm } from "./group-text-post-form";
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 py-2">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground">Posting to group</p>
          </div>
        </div>
        ...
        {postType === "text" && (
          <div className="space-y-4">
            ...
            <div className="border-t pt-4">
              ...
              <div className="flex gap-2 flex-wrap">
                <button
                  onClick={() => {
                    ...
                    const input = document.getElementById('group-image-upload-inline');
                ...
                <button
                  onClick={() => {
                    ...
                    const input = document.getElementById('group-video-upload-inline');
                    if (input) input.click();
                  }}
                ...
                onChange={(e) => {
                  ...
                  imageFiles.forEach((file) => {
                    if (file.size <= 25 * 1024 * 1024) {
                      useCreateGroupPostStore.getState().addMediaFile(file);
                    }
                  });
                  e.target.value = "";
                }}
              ...
              />
              <input
                id="group-video-upload-inline"
                type="file"
                accept="video/*"
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file && file.type.startsWith("video/") && file.size <= 300 * 1024 * 1024) {
                    useCreateGroupPostStore.getState().setVideoFile(file);
                  }
                  e.target.value = "";
                }}
                className="hidden"
        ...
        {postType === "text" && mediaFiles.length > 0 && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  useCreateGroupPostStore.getState().clearMediaFiles();
                }}
              ...
              {mediaFiles.map((file, index) => {
                ...
                return (
                  <div key={index} className="relative aspect-square rounded-lg overflow-hidden group">
                    ...
                    <button
                      onClick={() => {
                        useCreateGroupPostStore.getState().removeMediaFile(index);
                      }}
        ...
        {postType === "text" && videoFile && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => {
                  useCreateGroupPostStore.getState().setVideoFile(null);
                }}
                className="text-sm text-muted-foreground hover:text-foreground"
              >
                Remove
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) => useCreateGroupPostStore.getState().setMutedByDefault(e.target.checked)}
                  className="rounded"
                />
              ...
              <label className="flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={loop}
                  onChange={(e) => useCreateGroupPostStore.getState().setLoop(e.target.checked)}
                  className="rounded"
                />
