  ...
  const renderTextPost = () => {
    if (data.backgroundUrl && data.textStyle) {
      return (
        <div
          ...
          className="relative aspect-square w-full overflow-hidden rounded-2xl shadow-inner cursor-pointer block"
          style={{
    ...
    if (data.text) {
      ...
      return (
        <div>
          <p
            className="text-sm leading-relaxed text-foreground/90 whitespace-pre-wrap cursor-pointer"
            onClick={() => {
              if (!shouldTruncate) onOpen?.(post);
            }}
          >
            {displayText}
            {shouldTruncate && (
    ...
    return (
      <div className="mt-4 grid grid-cols-2 gap-2">
        {data.medias.map((media, index) => (
          ...
          >
            <MediaViewer media={media} layout="grid" />
          </div>
        ))}
      </div>
    );
